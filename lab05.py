...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    location = make_city(0,lat,lon)
    if distance(location, city2) > distance(location, city1):
...
def nut_finder(t):
    ...
    if label(t) == 'nut':
        return True
    return True in [nut_finder(a) for a in branches(t)]
...
def sprout_leaves(t, values):
    ...
    """
    if is_leaf(t):
        return tree(label(t),[tree(a) for a in values])
    else:
